                ...
                if (position + 1 < WIZARD_PAGE_SIZE) {
                    ...
                    if (editingTrigger.getTriggerID() == null) {
                        if (editingTrigger.getServerCode() != null) {
                            ...
                            wp.postNewTrigger(AppConstants.SCHEMA_NAME, AppConstants.SCHEMA_VERSION, editingTrigger.getActions(), editingTrigger.getPredicate(), editingTrigger.getCommandTarget()).then(new DoneCallback<com.kii.thingif.trigger.Trigger>() {
                                @Override
                                public void onDone(com.kii.thingif.trigger.Trigger result) {
                                    Toast.makeText(CreateTriggerActivity.this, "New Trigger is created. TriggerID=" + result.getTriggerID(), Toast.LENGTH_LONG).show();
                                    setResult(Activity.RESULT_OK);
                        ...
                        if (editingTrigger.getServerCode() != null) {
                            ...
                            wp.patchTrigger(editingTrigger.getTriggerID(), AppConstants.SCHEMA_NAME, AppConstants.SCHEMA_VERSION, editingTrigger.getCommandTarget(), editingTrigger.getActions(), editingTrigger.getPredicate()).then(new DoneCallback<com.kii.thingif.trigger.Trigger>() {
                                @Override
                                public void onDone(com.kii.thingif.trigger.Trigger result) {
                                    Toast.makeText(CreateTriggerActivity.this, "Trigger is updated. TriggerID=" + result.getTriggerID(), Toast.LENGTH_LONG).show();
                                    setResult(Activity.RESULT_OK);
